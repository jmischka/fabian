...
const optInterval = setInterval(function() {
...
function handleButton(e) {
	...
	function progress() {
		...
		if (timer < 100) {
			...
			audio.parentElement.offsetParent.parentElement.classList.remove('playing');
			return;
		}		
	}
...
window.addEventListener('scroll', function() {
	...
	contentSlides.forEach(slide => {
		if (slide.classList.contains('playing') && distanceScrolled < slide.offsetTop + slide.parentElement.offsetTop) {
			let audio = document.querySelector('.fabian-audioPlayer.playing').parentElement.children[1];
			audio.pause();
			clearInterval(interval);
			interval = null;
			audio.previousElementSibling.classList.remove('playing');
			audio.previousElementSibling.children[0].classList.remove('playing');
			audio.parentElement.offsetParent.parentElement.classList.remove('playing');
		} else if (slide.classList.contains('playing') && distanceScrolled > (slide.offsetTop + slide.parentElement.offsetTop) + slide.offsetHeight) {
			let audio = document.querySelector('.fabian-audioPlayer.playing').parentElement.children[1];
			audio.pause();
			clearInterval(interval);
			interval = null;
